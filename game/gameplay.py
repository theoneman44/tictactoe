...
class Gameplay:
    ...
    def get_winner(self, symbol: TicTacToeSymbol) -> None:
        b = self.board.board
        winner = ((b[0] == b[1] == b[2] == symbol) or
                  (b[3] == b[4] == b[5] == symbol) or
                  (b[6] == b[7] == b[8] == symbol) or
                  (b[0] == b[3] == b[6] == symbol) or
                  (b[1] == b[4] == b[7] == symbol) or
                  (b[2] == b[5] == b[8] == symbol) or
                  (b[0] == b[4] == b[8] == symbol) or
                  (b[2] == b[4] == b[6] == symbol))
        if winner is True:
            self.quit = True
    ...
    def get_input_msg(self, input_msg) -> Callable[[None], str] | None:
        ...
        if self.board.board[input_msg] != ' ':
            print('Ячейка уже занята, выберите другую: ')
            return None
        else:
            self.board.board[input_msg] = self.player.current_player
